...
export async function POST(request: NextRequest) {
  try {
    ...
    const cacheKey = cacheSystem.generateKey('mobile', url)
    ...
    const jobId = jobQueue.addJob('mobile', url, ip)

    return NextResponse.json({
      jobId,
      cached: false,
      message: 'Mobile check queued successfully'
    })
...
export async function GET(request: NextRequest) {
  ...
  if (job.status === 'completed' && job.result) {
    const cacheKey = cacheSystem.generateKey('mobile', job.url)
