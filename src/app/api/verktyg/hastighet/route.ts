...
export async function POST(request: NextRequest) {
  try {
    ...
    const cacheKey = cacheSystem.generateKey('performance', url)
    ...
    const jobId = jobQueue.addJob('performance', url, ip)

    return NextResponse.json({
      jobId,
      cached: false,
      message: 'Performance check queued successfully'
    })
...
export async function GET(request: NextRequest) {
  ...
  if (job.status === 'completed' && job.result) {
    const cacheKey = cacheSystem.generateKey('performance', job.url)
